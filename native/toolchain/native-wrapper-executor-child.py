...
"""Enter the wrapper-probe cgroup and limits before replacing this process."""
...
MARKER = "ziv-native-wrapper-executor-child-v1"
...
EXPECTED_PRESERVED_DESCRIPTORS = 11
...
def main(argv: list[str]) -> int:
    try:
        ...
        if len(argv) < 10 or argv[0] != MARKER or "--" not in argv:
            raise ValueError("invalid native wrapper executor child invocation")
        separator = argv.index("--")
        if separator != 8 or not argv[separator + 1 :]:
            raise ValueError("native wrapper executor child argument boundary is not exact")
        ...
        if len(set(all_passed)) != len(all_passed) or 255 in all_passed:
            raise ValueError("native wrapper executor child descriptors are not distinct")
        if (
            nofile_limit != EXPECTED_NOFILE_LIMIT
            or file_size_limit != EXPECTED_FILE_SIZE_LIMIT
            or core_limit != EXPECTED_CORE_LIMIT
        ):
            raise ValueError("native wrapper executor child limits are not locked")
        if (
            len(command) <= len(EXPECTED_COMMAND_PREFIX)
            or tuple(command[: len(EXPECTED_COMMAND_PREFIX)])
            != EXPECTED_COMMAND_PREFIX
        ):
            raise ValueError("native wrapper executor child command prefix is not locked")
        namespace_script = command[len(EXPECTED_COMMAND_PREFIX)]
        if namespace_script not in {
            f"/proc/self/fd/{descriptor}" for descriptor in preserved
        }:
            raise ValueError("namespace helper is not a preserved descriptor")
        if not Path(command[0]).is_absolute():
            raise ValueError("native wrapper executor child command is not absolute")
        if os.geteuid() != 0 or os.environ != EXPECTED_ENVIRONMENT:
            raise ValueError("native wrapper executor child identity or environment is not exact")
        expected_open = {0, 1, 2, *all_passed}
        if _open_descriptor_set() != expected_open:
            raise ValueError("native wrapper executor child inherited an unexpected descriptor")
        ...
        if _open_descriptor_set() != {0, 1, 2, *preserved}:
            raise ValueError("native wrapper executor child descriptor cleanup is incomplete")
        signal.pthread_sigmask(signal.SIG_UNBLOCK, CONTROL_SIGNALS)
        os.execve(command[0], command, EXPECTED_ENVIRONMENT)
    except (OSError, ValueError) as error:
        return _fail(str(error))
    return _fail("native wrapper executor child command unexpectedly returned")
