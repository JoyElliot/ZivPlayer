...
if [[ "$#" -ne 31 || "${EUID}" -ne 0 || "$$" -ne 1 ]]; then
  die "invalid native wrapper build namespace invocation"
fi
...
tmp_fd="${10}"
wrapper_fd="${11}"
namespace_helper_fd="${12}"
runner_helper_fd="${13}"
seccomp_helper_fd="${14}"
apt_identity="${15}"
sdk_identity="${16}"
workspace_identity="${17}"
source_identity="${18}"
output_identity="${19}"
home_identity="${20}"
tmp_identity="${21}"
wrapper_identity="${22}"
namespace_helper_sha256="${23}"
runner_helper_sha256="${24}"
seccomp_helper_sha256="${25}"
shift 25

declare -A seen_descriptors=()
for value in \
  "${apt_fd}" "${sdk_fd}" "${workspace_fd}" "${source_fd}" \
  "${output_fd}" "${home_fd}" "${tmp_fd}" "${wrapper_fd}" \
  "${namespace_helper_fd}" "${runner_helper_fd}" "${seccomp_helper_fd}"; do
...
for value in \
  ...
  "${tmp_identity}" "${wrapper_identity}"; do
  [[ "${value}" =~ ^[0-9]+:[0-9]+$ ]] || die "input identity is invalid"
done
[[ "${runner_helper_sha256}" =~ ^[0-9a-f]{64}$ ]] \
  || die "runner helper digest is invalid"
...
for value in "${apt_root}" "${sdk_root}" "${workspace_root}"; do
  [[ "${value}" =~ ^/[A-Za-z0-9._/-]+$ \
    ...
    && "${value}" != *$'\r'* \
    && "${value}" != *$'\t'* ]] \
    || die "native wrapper input path is invalid"
done
...
for namespace_name in mnt net pid uts ipc; do
  ...
  if [[ ! "${expected}" =~ ${namespace_pattern} \
    ...
    die "native wrapper build did not enter a private ${namespace_name} namespace"
  fi
done
[[ "$1" == "ziv-native-wrapper-build-namespace-execution-v1" ]] \
  || die "unknown native wrapper build namespace profile"
...
wrapper_source="/proc/self/fd/${wrapper_fd}"
namespace_helper_source="/proc/self/fd/${namespace_helper_fd}"
runner_helper_source="/proc/self/fd/${runner_helper_fd}"
...
check_identity "${wrapper_source}" "${wrapper_identity}" "wrapper descriptor"
...
check_identity "${workspace_root}/wrapper" "${wrapper_identity}" "wrapper path"
for path in \
  "${apt_source}" "${sdk_source}" "${workspace_source}" \
  "${source_source}" "${output_source}" "${home_source}" \
  "${tmp_source}" "${wrapper_source}"; do
  [[ "$(/usr/bin/stat -f -c '%t' -- "${path}")" == ef53 ]] \
    || die "native wrapper input is not on an ext filesystem: ${path}"
done
...
[[ "$(/usr/bin/sha256sum -- "${runner_helper_source}")" \
    == "${runner_helper_sha256}  ${runner_helper_source}" ]] \
  || die "runner helper differs from its pinned descriptor"
...
require_real_directory() {
  ...
  [[ -d "${path}" && ! -L "${path}" ]] \
    || die "native wrapper mountpoint is not a real directory: ${path}"
  [[ "$(/usr/bin/readlink -e -- "${path}")" == "${path}" ]] \
    || die "native wrapper mountpoint traverses a symbolic link: ${path}"
  [[ "$(/usr/bin/stat -c '%a %u %g' -- "${path}")" == "${expected_mode} 0 0" ]] \
    || die "native wrapper mountpoint metadata differs: ${path}"
}
...
require_real_directory "${workspace_root}/wrapper" 555
...
[[ -z "$(/usr/bin/find "${apt_root}/opt" -mindepth 1 -maxdepth 1 -print -quit)" ]] \
  || die "APT /opt must be empty before native wrapper composition"
[[ -z "$(/usr/bin/find "${apt_root}/mnt" -mindepth 1 -maxdepth 1 -print -quit)" ]] \
  || die "APT /mnt must be empty before native wrapper composition"
...
for path in "${apt_root}" "${sdk_root}" "${workspace_root}"; do
  if /usr/bin/findmnt --noheadings --mountpoint "${path}" >/dev/null 2>&1; then
    die "native wrapper input unexpectedly starts as a mountpoint: ${path}"
  fi
done
...
carrier_record="$(/usr/bin/findmnt --raw --noheadings --mountpoint "${carrier}" \
  ...
  || die "cannot inspect native wrapper carrier tmpfs"
[[ "${carrier_record}" != *$'\n'* ]] \
  || die "native wrapper carrier tmpfs has duplicate mount records"
carrier_target="${carrier_record%% *}"
carrier_tail="${carrier_record#* }"
[[ "${carrier_tail}" != "${carrier_record}" ]] \
  || die "native wrapper carrier record is incomplete"
...
if [[ "${carrier_target}" != "${carrier}" \
  ...
  die "native wrapper carrier tmpfs policy is not exact"
fi
...
for path in "${carrier}/upper" "${carrier}/work" "${new_root}"; do
  [[ "$(/usr/bin/stat -Lc '%a %u %g %d' -- "${path}")" \
    ...
    || die "native wrapper carrier path escaped tmpfs: ${path}"
done
...
/usr/bin/install -d -m 0755 "${new_root}/build/source"
/usr/bin/install -d -m 0555 "${new_root}/build/wrapper"
/usr/bin/install -d \
  -m 0700 \
...
check_identity "${wrapper_source}" "${wrapper_identity}" "wrapper bind descriptor"
check_identity "${workspace_root}/wrapper" "${wrapper_identity}" "wrapper bind path"
/usr/bin/mount --bind "${wrapper_source}" "${new_root}/build/wrapper"
/usr/bin/mount -o remount,bind,ro,nosuid,nodev,noexec \
  "${new_root}/build/wrapper"
check_identity "${new_root}/build/wrapper" "${wrapper_identity}" "wrapper bind target"
...
for name in null zero random urandom; do
  ...
  case "${name}" in
    ...
    random) minor=8 ;;
    urandom) minor=9 ;;
    *) die "unknown native wrapper device: ${name}" ;;
  esac
  mode=666
...
/usr/bin/install -m 0400 "${runner_helper_source}" \
  "${new_root}/run/ziv-native-wrapper-build-runner.bash"
[[ "$(/usr/bin/sha256sum -- "${new_root}/run/ziv-native-wrapper-build-runner.bash")" \
    == "${runner_helper_sha256}  ${new_root}/run/ziv-native-wrapper-build-runner.bash" ]] \
  || die "copied native wrapper runner helper digest changed"
...
if /usr/bin/grep -Eq '[[:space:]]/mnt(/|[[:space:]])' /proc/self/mountinfo \
  ...
  die "old host root remains visible after native wrapper pivot_root"
fi
...
require_mount_policy() {
  ...
  mount_record="$(/usr/bin/findmnt --raw --noheadings --mountpoint "${target}" \
    ...
    || die "cannot inspect native wrapper mount: ${target}"
  [[ "${mount_record}" != *$'\n'* ]] \
    || die "native wrapper mount has duplicate records: ${target}"
  filesystem="${mount_record%% *}"
  mount_tail="${mount_record#* }"
  [[ "${mount_tail}" != "${mount_record}" ]] \
    || die "native wrapper mount record is incomplete: ${target}"
  options="${mount_tail%% *}"
...
mount_inventory="$(/usr/bin/findmnt --raw --noheadings --output TARGET,FSTYPE)" \
  || die "cannot inventory native wrapper mounts"
...
while [[ -n "${remaining_mounts}" ]]; do
  ...
  [[ "${filesystem}" != "${mount_record}" && "${filesystem}" != *" "* ]] \
    || die "native wrapper mount inventory record is invalid"
  case "${target}:${filesystem}" in
    /:overlay|/build:tmpfs|/build/source:ext4|/build/output:ext4|/build/home:ext4|/build/tmp:ext4|/build/wrapper:ext4|/opt/zivplayer/toolchain:ext4|/proc:proc|/proc/keys:tmpfs|/dev:tmpfs|/dev/shm:tmpfs|/run:tmpfs|/tmp:tmpfs|/var/tmp:tmpfs|/var/log:tmpfs)
      [[ -z "${seen_mounts["${target}"]+present}" ]] \
        || die "duplicate native wrapper mount remains: ${target}"
      seen_mounts["${target}"]="${filesystem}"
      ;;
    *)
      die "unexpected native wrapper mount remains: ${target} (${filesystem})"
      ;;
  esac
done
[[ "${#seen_mounts[@]}" -eq 16 ]] || die "native wrapper mount set is incomplete"
...
if ! /usr/bin/awk '
  $5 != "/build/wrapper" { next }
  {
    count++
    delete options
    split($6, fields, ",")
    for (field_index in fields) options[fields[field_index]] = 1
    if (!options["ro"] || !options["nosuid"] || !options["nodev"] || !options["noexec"] || options["rw"] || options["exec"]) bad = 1
    separator = 0
    for (field_index = 7; field_index <= NF; field_index++) {
      if ($field_index == "-") { separator = field_index; break }
      if ($field_index ~ /^(shared|master|propagate_from):/) bad = 1
    }
    if (separator == 0 || $(separator + 1) != "ext4") bad = 1
  }
  END { if (count != 1 || bad) exit 2 }
' /proc/self/mountinfo; then
  die "native wrapper mountinfo policy is not exact"
fi
...
' /proc/net/dev; then
  die "native wrapper network namespace is not loopback-only"
fi
if ! /usr/bin/awk '
  NR == 1 { if ($1 != "Iface") exit 2; next }
  { exit 2 }
' /proc/net/route; then
  die "native wrapper build namespace exposes an IPv4 route"
fi
if ! /usr/bin/awk 'NF != 10 || $10 != "lo" { exit 2 }' /proc/net/ipv6_route; then
  die "native wrapper build namespace exposes an IPv6 route"
fi
...
[[ "$(/usr/bin/sha256sum -- "${apt_seccomp_helper}")" \
  ...
  || die "APT seccomp helper differs from the wrapper policy"
[[ "$(/usr/bin/sha256sum -- "${seccomp_helper_source}")" \
    == "${seccomp_helper_sha256}  ${seccomp_helper_source}" ]] \
  ...
  case "${descriptor}" in
    ...
    *)
      if [[ "${descriptor}" =~ ^[0-9]+$ \
        ...
        die "native wrapper build namespace helper retained an unexpected descriptor: ${descriptor}"
      fi
      ;;
  esac
done
(( ${#namespace_script_descriptors[@]} <= 1 )) \
  || die "native wrapper build namespace helper retained duplicate script descriptors"
...
# script descriptor here cannot prevent the final exec command from being read.
{
  ...
  exec /usr/bin/setpriv \
    ...
    /usr/bin/env -i \
      ...
      /run/ziv-native-wrapper-build-runner.bash --execute-locked-wrapper-build
}
