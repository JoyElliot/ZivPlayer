...
for value in \
  ...
  "${runner_helper_fd}" "${seccomp_helper_fd}"; do
...
for namespace_name in mnt net pid uts ipc; do
  ...
  if [[ ! "${expected}" =~ ${namespace_pattern} \
    ...
    die "native build executor did not enter a private ${namespace_name} namespace"
  fi
done
[[ "$1" == "ziv-native-build-namespace-execution-v1" ]] \
...
/usr/bin/install -m 0400 "${runner_helper_source}" \
  "${new_root}/run/ziv-native-build-runner.bash"
[[ "$(/usr/bin/sha256sum -- "${new_root}/run/ziv-native-build-runner.bash")" \
    == "${runner_helper_sha256}  ${new_root}/run/ziv-native-build-runner.bash" ]] \
  || die "copied native build executor helper digest changed"
...
for descriptor in \
  "${apt_fd}" "${sdk_fd}" "${workspace_fd}" "${source_fd}" \
  "${output_fd}" "${home_fd}" "${tmp_fd}" "${runner_helper_fd}"; do
