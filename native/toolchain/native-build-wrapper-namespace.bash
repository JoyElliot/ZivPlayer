...
if [[ "$#" -ne 31 || "${EUID}" -ne 0 || "$$" -ne 1 ]]; then
  die "invalid native wrapper namespace invocation"
fi
...
for value in \
  ...
  "${namespace_helper_fd}" "${probe_helper_fd}" "${seccomp_helper_fd}"; do
...
for namespace_name in mnt net pid uts ipc; do
  ...
  if [[ ! "${expected}" =~ ${namespace_pattern} \
    ...
    die "native wrapper probe did not enter a private ${namespace_name} namespace"
  fi
done
[[ "$1" == "ziv-native-build-wrapper-namespace-probe-v1" ]] \
  || die "unknown native wrapper namespace profile"
...
/usr/bin/install -m 0400 "${probe_helper_source}" \
  "${new_root}/run/ziv-native-build-wrapper-probe.bash"
[[ "$(/usr/bin/sha256sum -- "${new_root}/run/ziv-native-build-wrapper-probe.bash")" \
    == "${probe_helper_sha256}  ${new_root}/run/ziv-native-build-wrapper-probe.bash" ]] \
  || die "copied native wrapper probe helper digest changed"
...
if ! /usr/bin/awk '
  ...
  { exit 2 }
' /proc/net/route; then
  die "native wrapper namespace exposes an IPv4 route"
fi
if ! /usr/bin/awk 'NF != 10 || $10 != "lo" { exit 2 }' /proc/net/ipv6_route; then
  die "native wrapper namespace exposes an IPv6 route"
fi
  ...
  case "${descriptor}" in
    ...
    *)
      if [[ "${descriptor}" =~ ^[0-9]+$ \
        ...
        die "native wrapper namespace helper retained an unexpected descriptor: ${descriptor}"
      fi
      ;;
  esac
done
(( ${#namespace_script_descriptors[@]} <= 1 )) \
  || die "native wrapper namespace helper retained duplicate script descriptors"
...
# script descriptor here cannot prevent the final exec command from being read.
{
  ...
  exec /usr/bin/setpriv \
    ...
    /usr/bin/env -i \
      ...
      /run/ziv-native-build-wrapper-probe.bash --namespace-probe
}
